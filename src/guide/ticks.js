import { degree, unique } from './utils';
import { angle, sub } from '../geometry/utils';

export function ticksBottom(renderer, ticks, {
  tickLength,
  fontSize
}) {
  for (const {
...
export function ticksTop(renderer, ticks, {
  tickLength,
  fontSize
}) {
  for (const {
...
export function ticksLeft(renderer, ticks, {
  tickLength,
  fontSize
}) {
  for (const {
...
export function ticksCircular(renderer, ticks, {
  tickLength,
  fontSize,
  center
}) {
...
function rotationOf(center, [x, y]) {
  ...
  return {
    tickRotation: tickRotation - Math.PI / 2,
    textRotation
  };
}
